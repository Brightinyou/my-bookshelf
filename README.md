...
**PDF·TXT 문서를 Word 문서·Obsidian 위키 노트로 바꾸는 개인 연구 도구** — 텍스트 변환 → 챕터 분할 → (영문서) 한국어 번역 → 문서 요약 → **Word(.docx) 내보내기·Obsidian Wiki 반영(둘 다 가능)**를 한 흐름으로 처리합니다.
...
```
PDF/TXT  →  텍스트 변환  →  챕터 분할  →  영문번역(영문서만)  →  문서요약  →  Word(.docx) · Obsidian Wiki (둘 다 가능)
```
