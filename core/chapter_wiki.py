...
- ⭐⭐ 장 전체를 끝까지 고르게 다룬다. 원문에 소제목(절)이 있으면 그 구조를 따라 소제목을 세우고, 소제목 개수와 분량을 각 절의 원문 분량에 비례해 배분한다 — 원문의 절반을 차지하는 절이 요약에서 한두 항목으로 뭉개지면 안 된다. 앞부분에 소제목을 다 쓰고 뒷부분을 한 항목에 몰아넣지 말 것. 원문에 소제목이 없어도 뒷부분의 실천 방법·절차·단계 나열(예: 다섯 가지 방법)은 앞부분의 논증과 같은 비중으로 항목을 나눈다.
...
{{
  ...
  "body": "## {h_overview}\\n(이 장의 핵심 문제의식에서 출발해 어떤 주장을 거쳐 어떤 결론에 이르는지 {sent_ov}문장. 원문의 화자·질문·유보가 중요하면 보존하고, 원문 밖의 해석을 더하지 않는다.)\\n\\n## {h_main}\\n### (소제목 {n_sub}개 안팎)\\n(⭐ 분량 목표: '주요 내용' 전체를 원문(약 {src_chars}자)의 약 {pct}%인 약 {target_chars}자 분량으로 쓴다. 각 소제목은 대략 {per_sub}자 안팎으로, 원문에 실제 있는 개념 정의 → 논거·전제 → 근거·사례 → 결론 순서를 보존한다. 소제목은 원문의 절 구조와 각 절의 분량 비율을 따라 장 끝까지 고르게 배분한다(뒷부분 압축 금지). 원문에 없는 함의·평가·정책·배경 설명으로 분량을 채우지 말 것. 원문에 없는 내용을 쓸 바에는 더 짧게 쓸 것.)\\n\\n## {h_quotes}\\n| {c_topic} | {c_quote} |\\n|---|---|\\n\\n## {h_keywords}\\n(이 장에서 실제로 설명되거나 반복되는 핵심 개념 {n_kw}개만. 한 줄에 하나씩 '#키워드 — 원문 범위의 개념 해설 1문장' 형식. 원문에 없는 정의·효과를 더하지 말 것. 키워드는 공백 없는 {tgt}, 원어는 원문에 있을 때만 해설 쪽에 병기.)"
}}
