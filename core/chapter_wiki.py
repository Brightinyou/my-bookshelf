...
{lang_rule} 핵심 주장과 결론 중심. "저자는 ~한다" 같은 지칭 없이 내용으로 직접 서술한다(서로 다른 사상가를 대조할 때만 그 이름을 밝힌다). 장 요약에 없는 내용 지어내기 금지.
