...
- ★**줄바꿈은 문단이 바뀔 때만 한다.** 인쇄된 줄마다 바꾸지 마라. 한 문단은 한 줄로.
  **단, 아래 각주 규칙이 이보다 우선한다.**
- 각주 번호(위첨자 숫자)는 본문 해당 위치에 그대로 숫자로 적는다.
- ★★**쪽 아래 각주는 본문에 이어 붙이지 마라.** 본문을 다 적은 뒤 **빈 줄 하나를 두고**,
  각주 **하나에 한 줄씩** 적는다 — 줄 첫머리에 번호, 한 칸 띄우고 내용. 각주는 문단이
  아니므로 위의 '한 문단은 한 줄' 규칙을 여기에 적용하지 마라.
  잘못된 예: `…도취되지 않고 11 이기상, "현대 기술의 본질," 454. 12 박찬국, …`
  옳은 예:
  ```
  …도취되지 않고

  11 이기상, "현대 기술의 본질: 도발과 닦달," 『강연과 논문』, 454.
  12 박찬국, "하이데거와 니힐리즘의 극복," 『강연과 논문』, 406.
  ```
...
def assemble(pdf_path: Path, out_txt: Path, total_pages: int | None = None) -> Path:
    ...
    # ★각주가 본문 문단에 녹아든 쪽을 먼저 되돌린다 — 쪽을 잇기 **전에** 해야 한다.
    # 이어 붙이면 앞 쪽 끝줄과 다음 쪽 첫줄이 한 줄이 되어 엉뚱한 자리를 가른다.
    # (판독 프롬프트를 고쳤지만 이미 읽은 글도 되돌려야 한다. 실측 373쪽 중 82쪽 복구,
    #  글자 변화 0, 각주 인식 475→644개.)
    try:
        from services import footnotes as _fn0
        parts = [_fn0.split_inline_notes(x) for x in parts]
    except Exception as e:
        append_log(f"WARN: 각주 줄 되돌리기 실패 ({type(e).__name__}) {str(e)[:120]}")
